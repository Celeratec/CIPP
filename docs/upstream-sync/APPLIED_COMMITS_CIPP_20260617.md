...
| `1cd1ef7223672170bdce1fffe88d8bb4ddb903d9` | TBD | Applied with adaptation | TAP audit log template filter | `src/data/AuditLogTemplates.json` | JSON parse OK | Cherry-pick conflicted. Upstream also removed duplicate `ne []` SecuredAccessPassData filter — fork still retains that block (not changed). Manual apply: `like` Input `"*"` → `"*PassId*"` only. |
